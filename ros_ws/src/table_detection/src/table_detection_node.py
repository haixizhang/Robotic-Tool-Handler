...
from std_msgs.msg import MultiArrayDimension, Float32MultiArray, MultiArrayLayout
import numpy as np
...
class ObjectPointCloudPublisher:
    def __init__(self):
        ...
        self.position_pub = rospy.Publisher("/target_pub", Float32MultiArray, queue_size=10)
        self.center_new_pub = rospy.Publisher("/refine_center_point", Point, queue_size=10)
        # Subscribers
                ...
                if bbox.Class == self.target_name:
                    ...
                    # Adjust the bounding box to be slightly larger
                        ...
                        if object_points.size > 0:
                            ...
                            if segmented_points is not None and self.state == 2: 
                                self.state = 0   
                                # without segmentation
                                target_cloud_ros = self.numpy_to_ros_point_cloud(object_points,frame_id="camera_link",point_type="TARGET")
                                # segmentated
                                object_cloud_ros = self.numpy_to_ros_point_cloud(segmented_points, frame_id="camera_depth_optical_frame",point_type="OBJECT")
                                # table
                                table_cloud_ros = self.numpy_to_ros_point_cloud(table, frame_id="camera_depth_optical_frame",point_type="TABLE")
                                ...
                                ros_array.layout.dim = [MultiArrayDimension(label='rows', size=object_cloud_ros.shape[0], stride=object_cloud_ros.shape[0]*object_cloud_ros.shape[1]),
                                                        MultiArrayDimension(label='cols', size=object_cloud_ros.shape[1], stride=object_cloud_ros.shape[1])]
                                ros_array.layout.data_offset = 0  # you can set this to zero
                                ros_array.data = object_cloud_ros.ravel()  # Flatten the array
                                self.position_pub.publish(ros_array)
                                self.center_new_pub.publish(centerpoint_msg)
    ...
    def plane_segmentation(self,points):
        ...
        plane_model, inliers = pcd.segment_plane(ransac_n=50,num_iterations=3000, distance_threshold=0.1)
        [a, b, c, d] = plane_model

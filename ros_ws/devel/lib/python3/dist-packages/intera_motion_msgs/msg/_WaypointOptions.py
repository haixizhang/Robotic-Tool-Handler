...
"""
  ...
  def serialize(self, buff):
    ...
    try:
      ...
      _x = self.max_joint_speed_ratio
      buff.write(_get_struct_d().pack(_x))
      length = len(self.joint_tolerances)
      buff.write(_struct_I.pack(length))
      pattern = '<%sd'%length
      buff.write(struct.Struct(pattern).pack(*self.joint_tolerances))
      length = len(self.max_joint_accel)
      buff.write(_struct_I.pack(length))
      pattern = '<%sd'%length
      buff.write(struct.Struct(pattern).pack(*self.max_joint_accel))
      _x = self
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      self.joint_tolerances = s.unpack(str[start:end])
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      pattern = '<%sd'%length
      ...
      self.max_joint_accel = s.unpack(str[start:end])
      _x = self
      start = end
      end += 40

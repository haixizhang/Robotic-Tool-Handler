...
"""
  ...
  def serialize(self, buff):
    ...
    try:
      ...
      if python3 or type(_x) == unicode:
        ...
        length = len(_x)
      buff.write(struct.Struct('<I%ss'%length).pack(length, _x))
      _x = self.options.max_joint_speed_ratio
      buff.write(_get_struct_d().pack(_x))
      length = len(self.options.joint_tolerances)
      buff.write(_struct_I.pack(length))
      pattern = '<%sd'%length
      buff.write(struct.Struct(pattern).pack(*self.options.joint_tolerances))
      length = len(self.options.max_joint_accel)
      buff.write(_struct_I.pack(length))
      pattern = '<%sd'%length
      buff.write(struct.Struct(pattern).pack(*self.options.max_joint_accel))
      _x = self
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      if python3:
        self.options.label = str[start:end].decode('utf-8', 'rosmsg')
      else:
      ...
      self.options.joint_tolerances = s.unpack(str[start:end])
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      pattern = '<%sd'%length
      ...
      self.options.max_joint_accel = s.unpack(str[start:end])
      _x = self
      start = end
      end += 40

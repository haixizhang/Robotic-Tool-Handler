...
namespace intera_core_msgs
{
  ...
  class SEAJointState : public ros::Msg
  {
    public:
      ...
      typedef float _interaction_torque_type;
    ...
    SEAJointState():
      ...
      commanded_position_length(0), st_commanded_position(), commanded_position(nullptr),
      commanded_velocity_length(0), st_commanded_velocity(), commanded_velocity(nullptr),
      commanded_acceleration_length(0), st_commanded_acceleration(), commanded_acceleration(nullptr),
      commanded_effort_length(0), st_commanded_effort(), commanded_effort(nullptr),
      actual_position_length(0), st_actual_position(), actual_position(nullptr),
      actual_velocity_length(0), st_actual_velocity(), actual_velocity(nullptr),
      actual_effort_length(0), st_actual_effort(), actual_effort(nullptr),
      gravity_model_effort_length(0), st_gravity_model_effort(), gravity_model_effort(nullptr),
      gravity_only_length(0), st_gravity_only(), gravity_only(nullptr),
      interaction_torque_length(0), st_interaction_torque(), interaction_torque(nullptr),
      hysteresis_model_effort_length(0), st_hysteresis_model_effort(), hysteresis_model_effort(nullptr),
      crosstalk_model_effort_length(0), st_crosstalk_model_effort(), crosstalk_model_effort(nullptr),
      hystState(0)
    {
    }
    ...
    virtual int serialize(unsigned char *outbuffer) const override
    {
      ...
      *(outbuffer + offset + 3) = (this->interaction_torque_length >> (8 * 3)) & 0xFF;
      ...
      offset += serializeAvrFloat64(outbuffer + offset, this->interaction_torque[i]);
      }
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      if(interaction_torque_lengthT > interaction_torque_length)
        this->interaction_torque = (float*)realloc(this->interaction_torque, interaction_torque_lengthT * sizeof(float));
      interaction_torque_length = interaction_torque_lengthT;
      for( uint32_t i = 0; i < interaction_torque_length; i++){
      offset += deserializeAvrFloat64(inbuffer + offset, &(this->st_interaction_torque));
        memcpy( &(this->interaction_torque[i]), &(this->st_interaction_torque), sizeof(float));
      }
    ...
    virtual const char * getType() override { return "intera_core_msgs/SEAJointState"; };
    virtual const char * getMD5() override { return "5df7bfe6612daccc934b34dad7800762"; };
